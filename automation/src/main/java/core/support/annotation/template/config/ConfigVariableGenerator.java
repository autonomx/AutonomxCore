...
public class ConfigVariableGenerator {
	...
	private static void writeConfigVariableClassImplementation() throws IOException {
		...
		JavaFileObject fileObject = MainGenerator.PROCESS_ENV.getFiler()
				.createSourceFile(PackageHelper.CONFIG_MANAGER_PATH + "." + CONFIG_VARIABLE);
		 ...
		 * 
		 * public static ConfigManager globalParallelTestCount() { String value =
		 * Config.getValue("global.parallel.testCount"); return new
		 * ConfigModule(value,"1"); }
		 * 
		 * public void setValue(String key, String value){ Config.putValue(key,
		 * value.toString()); } }
		 */
		...
		/**
		 * public static ConfigManager parallelTestCount() { return new
		 * ConfigManager("parallelTestCount"); }
		 * 
		 */
