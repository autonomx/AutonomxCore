...
public class CsvDataObject {
	 ...
	 * 
	 * public User withUserName(String username) { this.username = username; return
	 * this; }
	 * 
	 * public User withPassword(String password) { this.password = password; return
	 * this; }
	 * 
	 ...
	 * 
	 * public User admin() { User login = new User().withUserName("admin" +
	 * Helper.generateRandomString(4)).withPassword("password1"); return login; }
	 * 
	 * @DataProvider(name = "DataRunner") public synchronized Iterator<String[]>
	 *                    dataProvider() { List<String[]> testCases =
	 *                    Helper.csv_getAllCsvDataFirstRowAsHeader(PropertiesReader.getLocalRootPath()
	 *                    + "⁨src/main/java/module/webApp/data/User.csv"); return
	 *                    testCases.iterator(); } }
	 * @param file
	 * @throws Exception
	 */
...
//		}
		...
		/*
		 * public User admin() { User user = new User(); user.username = "autoAdmin1";
		 * user.password = "autoPass1"; user.name = "dave"; user.id = "admin"; return
		 * user; }
		 */
		...
		/*
		 * // update value in csv file public synchronized void updateRate( String rate)
		 * {
		 * 
		 * Helper.
		 * assertTrue("id cannot be empty, select row id. eg. Data.webApp.users().admin().updateName('bob'); "
		 * , !this.id.isEmpty());
		 * 
		 * String path =
		 * "/Users/Shared/Jenkins/Documents/Selenium/mining/mining/automation/src/main/java/module/webApp/data";
		 * String fileName = "MinerList.csv"; String value = rate; String[] valueArray =
		 * value.split(","); int columnIndex = Helper.csv.getColumnIndex("rate", path +
		 * "/" + fileName); int rowIndex = Helper.csv.getRowIndex(this.id, path + "/" +
		 * fileName ); CsvObject csv = new
		 * CsvObject().withCsvPath(path).withCsvFile("MinerList.csv").withValue(
		 * valueArray).withRow(rowIndex).withColumn(columnIndex);
		...
		for (int i = firstIndex; i < headers.size(); i++) {
			...
			bw.append(
					"		Helper.assertTrue(\"id cannot be empty, select row id. eg. Data.webApp.users().admin().updateName('bob'); \" , !this.id.isEmpty());"
							+ "\n" + "\n");
			bw.append("		String path = \""
					+ file.getParent().replace(File.separator, File.separator + File.separator) + File.separator
					+ File.separator + "\";" + "\n");
			...
			bw.append("		CsvObject csv = new CsvObject().withCsvPath(path).withCsvFile(\"" + file.getName()
					+ "\").withValue(valueArray).withRow(rowIndex).withColumn(columnIndex);" + "\n");
...
//		}
		...
		/**
		 * public void addRow(String type, String hashrate) { String path =
		 * Helper.getRootDir() + "src" + File.separator + "main" + File.separator +
		 * "java" + File.separator + "module" + File.separator + "webApp" +
		 * File.separator + "data" + File.separator;
		 * 
		 * String[] value = "type,hashrate".split(","); CsvObject csv = new
		 * CsvObject().withCsvPath(path).withCsvFile("MinerList.csv").withValue(value);
	...
	/**
	 * replaces duplicate ids with id + index allows for data with duplicate ids to
	 * compile user should replace duplicates with unique values
	 * 
	...
	/**
	 * removes the first column in the row. this is for the id colum that needs to
	 * be removed from data provider
	 * 
	 * @param rowList
	 * @return
	 */
	...
	/**
	 * updates the rows to add missing values if missing eg. @id, name, password bob
	 * -> becomes bob, ""
	 * 
	 * @return
	 */
	...
	/**
	 * gets list of column headers replaces '@id' if exists with id
	 * 
	 * @param file
	...
	/**
	 * converts list of headers to method parameters eg. {"id","type"} becomes
